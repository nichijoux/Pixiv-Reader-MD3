...
| RankingList | RankingList.kt | 通用排行榜（ScrollableTabRow + HorizontalPager 滑动切段 + 三态 + 触底加载，`itemContent(T, rank)` slot；调用方经 `stateFor(mode)` 提供每段独立 PagedState，数据驻留 VM） |
...
- **分段**：`ScrollableTabRow`（label 走 `RankingModeInfo.labelRes`）+ `HorizontalPager` 左右滑动切换；点 Tab 平滑滑动；滑段后回调 `onModeSelect` 触发该段加载；每段独立 `PagedState`（`stateFor`），已加载段切回不重复请求、无过渡动画。
