...
- **模块清单**（settings.gradle.kts）：`:app` + `:lib:pixivapi` + core（`common`/`network`/`database`/`datastore`/`ui`/`novel`/`comment`/`ranking`）+ feature 15 个（`auth`/`home`/`discover`/`illust`/`viewer`/`novel`/`reader`/`user`/`bookmark`/`watchlist`/`notification`/`manga`/`follow`/`onboarding`/`fanbox`）。
