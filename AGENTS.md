...
- `feature/download` 源码与 build.gradle 已删除、`settings.gradle` 已不注册（仅剩陈旧 `build/` 产物目录，可手动删除）；下载管理实现在 `feature:user`。
