 ...
 * - 小说详情：`pixiv.net/novel/show.php?id={id}[#页码]`（忽略 fragment，跳详情页）
 * - 小说系列：`pixiv.net/novel/series/{id}`
 * - 插画 / 漫画：`pixiv.net/artworks/{id}`、`pixiv.net/manga/{id}`
 * - 用户主页：`pixiv.net/users/{id}`、`pixiv.net/user/{id}/…`
 *
...
object PixivUrlParser {
    ...
    private val LINK_REGEX = Regex(
        """(?:https?://)?(?:www\.)?pixiv\.net/(?:novel/show\.php\?(?:[^#\s]*&)?id=|novel/series/|artworks/|manga/|users?/)(\d+)""",
    )
    ...
    fun parse(text: String): PixivLink? {
        ...
        val type = when {
            "/novel/show.php" in match.value -> PixivLinkType.NOVEL
            "/novel/series/" in match.value -> PixivLinkType.SERIES
            "/artworks/" in match.value || "/manga/" in match.value -> PixivLinkType.ILLUST
